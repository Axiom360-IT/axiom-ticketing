...
2. `previewCustomerImport` validates each row (zod) and flags `duplicate_in_file`, `duplicate_existing_customer`, `duplicate_existing_staff`, and `invalid` rows. It also resolves each row's email domain against known organizations (`resolveDomainsForImport`); free-mail domains (gmail, outlook, …) never suggest creating an org. Domains that don't match an existing org are grouped into `unmatchedDomains`, and the admin decides per-domain: create a new org (with a suggested name + abbreviation, `suggestOrgCode` calls run concurrently across all unmatched domains via `Promise.all` rather than one-at-a-time), assign to an existing org, or skip.
   - **Phone normalization** (`lib/customer-import/phone-normalize.ts:normalizeImportPhone`, via `react-phone-number-input`'s re-exported `parsePhoneNumber`): any reasonably-formatted pasted value — with or without its own country code — is parsed against the selected default country and stored as E.164. An unparseable phone never invalidates the row; it's dropped (phone stays optional) with a non-blocking `phoneWarning` shown in the preview table instead.
...
5. `process-customer-import-batch` provisions every row via the same `users/provision.ts` core `createUser` uses, and each new customer gets the customer-invite email (§22) — kept out of the request/response cycle so a large file can't time out the admin's browser. The invite send itself retries once before giving up (§22), and a permanent failure is trackable/bulk-actionable from the Users list (§6.1, §9).
...
- **Admin-provisioned invite (`createUser`'s Customer branch, and the customer-import batch, §17):** uses its own HMAC token (`signCustomerInviteToken`, `lib/tokens.ts`) and its own `customer_invite.expiry_hours` setting (default 72h) — separate from staff's Better Auth reset-token flow. `sendCustomerSetupInvite` (`lib/customer/invite.ts`) refreshes `users.invitedAt`/`inviteExpiresAt` from the LIVE setting on every (re)send, retries the actual email once immediately on failure (most send failures at this layer are a transient provider hiccup), and on a still-failed second attempt stamps `users.inviteSendFailedAt` — cleared automatically the moment any later send (retried or manual) succeeds. `acceptCustomerInvite` (`src/app/actions/customer-invite.ts`) consumes the token to set a password and sign the customer in. `computeInviteStatus` (`lib/users/invite-status.ts`) derives `active`/`invited`/`invite_expired`/`invite_failed` for the admin Users list — the first three purely from the clock (no stored status column), `invite_failed` from the one column that DOES need to be stored, since whether the last send attempt failed isn't something a clock can recompute. The Users list can filter to `invite_failed` (or `invited`/`invite_expired`) and bulk-resend to a multi-selected set of external/customer rows (`bulkResendCustomerInvites`, §9) — the same control also covers a customer who simply forgot their password, not just a failed-send case.
