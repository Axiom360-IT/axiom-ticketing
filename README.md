...
| Module | Tables | Notes |
|---|---|---|
| `auth.ts` | `users`, `accounts`, `sessions`, `verifications`, `passkeys` | Better Auth core + app-specific columns (`language`, `phone` — optional E.164 string used by the SMS dispatch leg; `createdById`, `isActive`, `deactivatedAt`, `lockedUntil`, `lastLoginAt`, `provisionedAt` — migration `pnpm db:add-provisioned-at-column`, set once a row's `accounts`/role rows are in place; `NULL` is the "still provisioning" signal for a bulk-import stub row (§17), checked first in `computeInviteStatus`, §22.2 — `invitedAt`/`inviteExpiresAt`/`inviteAcceptedAt`, `inviteSendFailedAt` — migration `pnpm db:add-invite-send-failed`, nullable, set only when a customer invite/reset email fails to send after one retry, §22.2). Phone is collected on the customer sign-up form, the customer profile, the admin user-create form, and the admin profile (all optional). Empty/null = no SMS for that user; the `dispatchNotification` function already gates on `r.phone` being truthy. |
...
| `procurement.ts` | `procurement_requests` | Four single-select stages (CR-26): `awaiting_customer_payment` → `order_pending` → `order_placed` → `order_completed`. Type ∈ {hardware, software, other}. **No approval/urgency** (removed in `0010`); stage-change actor/time lives in the audit log. `vendor` stays plain free text — see `vendors.ts` below |
| `vendors.ts` | `vendors` | Admin-managed vendor list (added 2026-08-19) backing the procurement-form picker (`VendorSelect`, §10) at `/admin/vendors`. `name` (unique, case-insensitive) + `is_active` (deactivate, never delete, so past requests keep their label) + `created_by_id`. Deliberately **not** a foreign key from `procurement_requests.vendor` — the picker offers these names as suggestions, but a requester can always type a one-off vendor that never joins the managed list, and editing an existing request's vendor (`updateProcurementVendor`) never rewrites this table. Provisioned/seeded via `pnpm db:add-vendors` (this repo's post-0018 convention — see §5.2) |
...
> **Heads-up for AI coding agents (found 2026-08-19):** drizzle-kit's own bookkeeping (`src/lib/db/migrations/meta/_journal.json` + the per-migration snapshot files) hasn't been updated since migration `0018` — every schema change from `0019` onward (categories, types, reviews, email-refs, audit presets, MCP tokens, on-hold status, invite-status columns, vendors, …) was applied through a **hand-written, idempotent one-shot script** under `src/lib/db/` (`add-*.ts`, run via its own `pnpm db:add-*` command — see §29), not through `pnpm db:migrate`. The matching `.sql` file still gets added to `migrations/` for documentation/history, but it's illustrative, not what's actually applied. **Do not run `pnpm db:generate`** for a new table/column — it diffs against the stale `0018` snapshot and will try to re-`CREATE TABLE` everything added since, which already exists live. Instead: write the schema file, hand-write a `CREATE TABLE IF NOT EXISTS … / ALTER TABLE …` idempotent script following the `add-vendors.ts` / `add-ticket-categories.ts` pattern, wire it to a `pnpm db:add-<thing>` script, and run that against `DATABASE_URL`.
...
- `(gated)/` — every route below this segment is protected by `layout.tsx`, which (a) requires a valid Better Auth session, (b) enforces the 12-hour idle timeout by inspecting `session.updatedAt`, (c) renders the sidebar + topbar + impersonation banner, and (d) hosts the `<main id="main-content">` landmark
  ...
  - `users/`, `users/new`, `users/[id]`, `users/import` — the customer-import wizard (§17); paste/upload a CSV of customers, resolve orgs, commit via background batch. The list itself (Internal/External tabs) has an "Invite status" filter (All/Failed to send/Pending/Expired, §22.2) alongside the existing Status filter; the External tab additionally gets a checkbox column + a "Resend invite" bulk-action bar (`bulkResendCustomerInvites`, §9) for multi-selecting rows and resending in one go — covers both a failed-send and a plain forgot-password case
...
| `customer-import/` | Bulk customer-CSV import support (§17): `domain-guess.ts` (free-mail deny-list + `guessOrgNameFromDomain`), `domain-resolution.ts` (`resolveDomainsForImport` — batch-resolves distinct email domains against known orgs in one query), `parse-rows.ts` (`parseImportRows` — quote-respecting CSV/TSV line parser + optional-header-row detection; pure, unit-tested, shared by the client wizard), `phone-normalize.ts` (`normalizeImportPhone` — parses a pasted phone against a caller-given default country via `react-phone-number-input`'s re-exported `parsePhoneNumber`, returns E.164 or a graceful failure, never throws) |
...
| `users/invite-status.ts` + `users/provision.ts` | `computeInviteStatus` derives `active`/`provisioning`/`invited`/`invite_expired`/`invite_failed` from `users.provisioned_at`/`invited_at`/`invite_expires_at`/`invite_accepted_at`/`invite_send_failed_at`. `provision.ts` holds three primitives: `provisionUser` (the synchronous `users`+`accounts`+roles insert `createUser`/`createUserViaMcp` use, immediately setting `provisionedAt`), `createCustomerImportStubs` (bulk-import's synchronous half — one bare `users` row per row, `onConflictDoNothing` on email for race safety, `provisionedAt` left null), and `finishCustomerProvisioning` (bulk-import's async half, called from the Inngest batch processor — grants roles, creates `accounts`, sets `provisionedAt` on an already-existing stub) |
...
| `process-customer-import-batch` | event `customer-import/batch.requested` | Finishes provisioning every row's already-existing `users` stub (created synchronously by `commitCustomerImport`, §17) via `finishCustomerProvisioning` and sends the invite email, outside the request/response cycle so large batches don't time out |
...
| `customer-import.ts` | `previewCustomerImport(rows, defaultCountry?)` (zod-validates + flags duplicates/domain matches + normalizes phone against `defaultCountry`, capped at 500 rows, client-editable before commit), `commitCustomerImport(rows, decisions, defaultCountry?)` (re-validates server-side by re-running preview, creates any admin-approved new orgs synchronously, resolves the Customer role id up front — aborting before any row exists if it's missing — bulk-creates every row's `users` stub synchronously via `createCustomerImportStubs`, then fires one `customer-import/batch.requested` event carrying each row's `userId` for the async job to finish). Both gated on `users.create` |
...
| `users.ts` | `createUser` — bypasses `auth.api.signUpEmail` and **inserts the `users` + `accounts` rows directly via Drizzle in a single `transactional`** (`accounts.password = null` until the user completes the setup-invite email). This avoids the session-issuing side effect of `signUpEmail` entirely, so the calling admin keeps their session no matter the cookie name Better Auth chooses across environments. A Customer-role target sends the customer-specific invite token (`sendCustomerSetupInvite`, §22) instead of Better Auth's staff reset-token flow. Plus the standard checks (cannot-grant-what-you-don't-have on role assignment; Super Admin grants require re-auth freshness; duplicate-email pre-check). Also: `updateUser`, `deactivateUser` / `reactivateUser`, `resetUserPassword` (staff → Better Auth `requestPasswordReset`; customer → `sendCustomerSetupInvite`, §22.2), `bulkResendCustomerInvites(userIds)` (loops `resetUserPassword` per selected user rather than reimplementing it — every permission check, role branch, and audit entry stays identical to a single resend; one bad id doesn't sink the rest of the batch; capped at 200 ids, rate-limited via `bulkResendInvites`; powers the Users list's multi-select bulk action, §6.1). `resetUserPassword` (and its MCP mirror `resetUserPasswordViaMcp`) refuse to act on a still-`provisioning` row (`users.provisionedAt IS NULL` — no `accounts`/role rows yet to reset). Also: `unlockUser` (clears Redis + DB lockout), `getDescendants` (BFS over `createdById`, depth-capped at 50). |
...
| `procurement.ts` | `createProcurementRequest` (`procurement.create` — Technician or Customer), `setProcurementStatus` (`procurement.manage` — moves the 4-stage pipeline directly, no approval step), `updateProcurementVendor` (added 2026-08-19 — corrects/sets a request's vendor after creation; gated on `procurement.manage` **or** `procurement.update` scoped to the requester's own request, reviving a permission that previously had no caller), plus `listProcurementForTicket`/`listProcurementForAdmin`/`getProcurementDetail` reads. `procurement.submitted` (on create) and `procurement.delivered` (on `order_completed`) each additionally CC the accountant contact list from Settings (`getAccountantRecipients()`, §15) by direct `sendEmail` — accountants aren't app users, so they're outside the per-user `notification/dispatch` preference system. Note: the `procurement_approval_threshold` setting, and the `procurement.approved`/`procurement.rejected` notification types + templates, are leftover from a pre-CR-24 approval workflow and are no longer wired to anything |
...
- `users/` — `create-user-form`, `edit-user-form`, `account-actions`, `deactivate-modal`, `impersonate-button`, `role-multi-select`, `user-row-actions`, `users-table` (DataTable-based list; `enableBulkActions` — External tab only — adds a checkbox column + bulk "Resend invite" bar, local `selectedIds` state, §6.1), `customer-import-wizard` (§17)
- `roles/` — `create-role-form`, `edit-role-form`, `permissions-matrix`, `role-row-actions`
- `procurement/` — `status-badge`, `request-form`, `ticket-section`, `decision-buttons`, `procurement-row-actions`, `vendor-select` (search-or-type-custom combobox, fetches `listActiveVendors()` client-side; used by both the create form and the edit control below), `vendor-editor` (inline pencil-to-edit control on the request detail page, gated on `procurement.manage` or own-request `procurement.update`)
- `settings/` — `branding-form`, `business-hours-form`, `sla-form`, `role-checklist-form` (generic staff-role checklist persisted as a `string[]` setting; backs the SLA warning/breach notify-role pickers, §13), `holidays-list`, `string-list-form`, `scalar-form`, `rate-limit-form`, `save-button`, `taxonomy-table` + `add-taxonomy-button` (shared by the categories and types admin pages; `ticket-type-icon-picker` — a `Popover`-based grid of the 30 curated icons (§5.1) — renders inside both **only when `kind === "type"`**, since categories have no icon column), `vendor-table` + `add-vendor-button` (the vendors admin page — a separate, simpler pair rather than a third `taxonomy-table` kind, since vendors have no order/default/icon columns)
...
**Role scoping** (`src/lib/notifications/audience.ts`) decides which events a role may see/tune in its profile (`STAFF_EVENT_TYPES`/`CUSTOMER_EVENT_TYPES`, split into `BASE_STAFF_EVENT_TYPES` for any staff member vs. `MANAGEMENT_EVENT_TYPES` for Coordinator/IT Director/Super Admin). Since 2026-08-19, `OVERSIGHT_ROLES = ["Coordinator","Super Admin"]` is additionally CC'd as an actual dispatch recipient on every staff-facing event type — not just the events that already broadcast to the full management trio — so a Coordinator or Super Admin sees the full staff-facing event set by default (each can still dial an individual event back to zero via their own preferences). IT Director's reach and every customer-facing event are unchanged. Full per-event routing + rationale: [`docs/notification-matrix.md`](./docs/notification-matrix.md), decision log: `DECISIONS.md`.
...
1. Admin pastes or uploads a CSV/TSV of customers (name, email, phone) — up to 500 rows, parsed by `parseImportRows` (`lib/customer-import/parse-rows.ts`), which respects double-quoted fields (so a comma inside a quoted name doesn't split the row when comma is also the delimiter) and detects/skips an optional header row. Also picks a **default country** (a `<Select>` of all ISO countries + calling codes, defaulting to the geo-resolved country via `useDefaultCountry()`) used only to interpret a phone number that doesn't already carry its own country code.
2. `previewCustomerImport` validates each row (zod) and flags `duplicate_in_file`, `duplicate_existing_customer`, `duplicate_existing_staff`, and `invalid` rows. It also resolves each row's email domain against known organizations (`resolveDomainsForImport`); free-mail domains (gmail, outlook, …) never suggest creating an org. Domains that don't match an existing org are grouped into `unmatchedDomains`, and the admin decides per-domain: create a new org (with a suggested name + abbreviation, `suggestOrgCode` calls run concurrently across all unmatched domains via `Promise.all` rather than one-at-a-time), assign to an existing org, or skip.
   - **Phone normalization** (`lib/customer-import/phone-normalize.ts:normalizeImportPhone`, via `libphonenumber-js/min`'s `parsePhoneNumber` directly — **not** `react-phone-number-input`, whose root export also constructs its React `<PhoneInput>` component graph at module load and has no place in a server action's bundle, see DECISIONS.md 2026-08-14): any reasonably-formatted pasted value — with or without its own country code — is parsed against the selected default country and stored as E.164. An unparseable phone never invalidates the row; it's dropped (phone stays optional) with a non-blocking `phoneWarning` shown in the preview table instead.
3. The admin reviews/edits the preview client-side, then confirms.
4. `commitCustomerImport` **re-validates everything server-side** (never trusts the client preview), synchronously creates any admin-approved new organizations, resolves the Customer role id (`loadCustomerRoleId`, aborting the whole commit if it's missing — before any row exists), then calls `createCustomerImportStubs` (`lib/users/provision.ts`) to bulk-insert every row's bare `users` record **immediately** — visible on the admin Users list right away as `provisioning` (§6.1, §22.2) — before firing a single `customer-import/batch.requested` event carrying each row's now-known `userId`. A row that loses a genuine concurrent-duplicate race on `users.email` (`onConflictDoNothing`) is excluded from the event and counted in the result's `skippedRaceDuplicate`, distinct from the deterministic `skippedDuplicate` found during re-validation.
5. `process-customer-import-batch` finishes each already-existing stub row via `finishCustomerProvisioning` (role grant, `accounts` row, `provisionedAt`) — the same transactional core `provisionUser` uses, just against a row that already exists — then sends the customer-invite email (§22). Provisioning + send both happen out of the request/response cycle so a large file can't time out the admin's browser. The invite send itself retries once before giving up (§22), and a permanent failure is trackable/bulk-actionable from the Users list (§6.1, §9). A row that fails to finish provisioning is stuck (visibly, as `provisioning`) rather than invisible — but unlike before this split, re-importing the same email won't retry it (it's now a known `duplicate_existing_customer`); it needs a manual fix.
...
- **Admin-provisioned invite (`createUser`'s Customer branch, and the customer-import batch, §17):** uses its own HMAC token (`signCustomerInviteToken`, `lib/tokens.ts`) and its own `customer_invite.expiry_hours` setting (default 72h) — separate from staff's Better Auth reset-token flow. `sendCustomerSetupInvite` (`lib/customer/invite.ts`) refreshes `users.invitedAt`/`inviteExpiresAt` from the LIVE setting on every (re)send, retries the actual email once immediately on failure (most send failures at this layer are a transient provider hiccup), and on a still-failed second attempt stamps `users.inviteSendFailedAt` — cleared automatically the moment any later send (retried or manual) succeeds. `acceptCustomerInvite` (`src/app/actions/customer-invite.ts`) consumes the token to set a password and sign the customer in. `computeInviteStatus` (`lib/users/invite-status.ts`) derives `active`/`provisioning`/`invited`/`invite_expired`/`invite_failed` for the admin Users list — `provisioning` (checked right after `inviteAcceptedAt`, before every other branch) covers a bulk-import row between its synchronous stub insert and the async job finishing it (`users.provisionedAt IS NULL`, migration `pnpm db:add-provisioned-at-column`, §17); the rest are purely clock-derived (no stored status column) except `invite_failed`, since whether the last send attempt failed isn't something a clock can recompute. `resetUserPassword`/`resetUserPasswordViaMcp` refuse to act on a still-`provisioning` row (no `accounts`/role rows yet), and the Users list disables bulk-select + hides Edit/Deactivate for one. The Users list can filter to any of these statuses and bulk-resend to a multi-selected set of external/customer rows (`bulkResendCustomerInvites`, §9) — the same control also covers a customer who simply forgot their password, not just a failed-send case.
...
| `pnpm db:add-vendors` | Idempotent standalone provisioning + seed of the `vendors` table (§5.1) — same post-0018 one-shot-script convention as the row above, not the drizzle-kit migration journal (see §5.2) |
...
- **2026-08-17 — Bulk-imported customers get a synchronous, visible "provisioning" stage:** `commitCustomerImport` now creates every row's `users` stub synchronously (`createCustomerImportStubs`) before the Inngest event goes out, instead of the async job creating it from scratch — so an imported customer is visible on the Users list (as a new `provisioning` invite-status) the instant the import is committed, not whenever the background job gets to them. `provisionUser` (staff/single-customer creation) is otherwise unchanged. New `users.provisioned_at` column (`pnpm db:add-provisioned-at-column` — backfill is one-time only, see `DECISIONS.md`).
- **2026-08-17 — Production hotfix: server code must not import `react-phone-number-input`'s root export:** a live 500 on every customer-import commit traced to `phone-normalize.ts` pulling in that package's full class-based `<PhoneInput>` component graph via its root export — broke under Turbopack's SSR bundling. Fixed by importing `libphonenumber-js/min` (the pure parsing engine) directly. Full rationale in `DECISIONS.md`.
- **2026-08-14 — Customer-import hardening:** four fixes to the bulk-import feature (§17) — phone numbers are now auto-detected against an admin-picked default country instead of strict-regex-or-reject-the-whole-row; CSV/TSV parsing respects quoted fields; per-domain org-code suggestions run concurrently; and a new `users.invite_send_failed_at` column + `invite_failed` status makes failed invite sends filterable and bulk-resendable from the Users list (which also covers a plain forgot-password case). Full rationale in `DECISIONS.md`.
...
- **2026-08-19 — Coordinator + Super Admin see every staff-facing notification:** `OVERSIGHT_ROLES` (`audience.ts`) is now CC'd on every staff-facing `notification/dispatch` — a deliberate deviation from several req-6.1–6.4 fallback-only/single-role rules (`ticket.reassigned`, `ticket.escalated`, `ticket.customer_replied`, `ticket.csat_unsatisfied`, `procurement.submitted`/`delivered`, `attachment.quarantined`). Also added the one previously-untoggleable event (`ticket.message_held`) to the preferences grid, and added SMS to 5 event types that only had email/in-app before. IT Director and every customer-facing event are unchanged. See `docs/notification-matrix.md` and DECISIONS.md for the full per-event breakdown and the reasoning for what was deliberately left out.
- **2026-08-19 — Vendor management + editable procurement vendor + accountant CC:** new `vendors` table/`/admin/vendors` admin page backs a search-or-custom-entry picker on the procurement form (`procurement_requests.vendor` stays free text, not a foreign key). New `updateProcurementVendor` action revives the previously-uncalled `procurement.update` permission (own-request-only) alongside `procurement.manage` (any request). `procurement.submitted`/`procurement.delivered` now also CC the Settings-configured accountant list. See DECISIONS.md for the dead-code findings (`procurement_approval_threshold`, stale migration journal) surfaced along the way.
