...
| Editor | TipTap 3 (`@tiptap/starter-kit` + link extension); sanitized server-side with isomorphic-dompurify |
...
| Module | Tables | Notes |
|---|---|---|
| `auth.ts` | `users`, `accounts`, `sessions`, `verifications`, `passkeys` | Better Auth core + app-specific columns (`language`, `phone`, `createdById`, `isActive`, `deactivatedAt`, `lockedUntil`, `lastLoginAt`) |
...
- `portal/(authenticated)/` — customer-role-only group; layout redirects non-Customers to `/admin`
...
| `messages/` | `sanitize.ts` (DOMPurify allowlist matching what TipTap can produce, `<a>` is forced to `target="_blank" rel="noopener noreferrer"` and `javascript:` / `data:` URLs are blocked), `visibility.ts` (SQL predicate hiding internal notes from customer queries) |
