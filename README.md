...
| AI agent access | `@modelcontextprotocol/sdk` — a Bearer-token-authenticated MCP server exposing read-mostly ticket tools (see §23) |
...
| `ticket-types.ts` | `ticket_types` | Same shape as `ticket_categories` (migration `0025`) for the independent "type of work" taxonomy (Service Request/Incident/Change/Project/Alert, seeded). Managed at `/admin/types` |
...
| `mcp-tokens.ts` | `mcp_tokens` | Migration `0030`. Self-service Bearer tokens (`axmcp_…` prefix, SHA-256 hashed, revocable) a user mints from their own profile to authenticate the MCP server (§23) as themselves |
...
| `mcp/` | `server.ts` (`buildMcpServer` — registers the MCP tool set: `get_ticket`, `list_tickets_by_status`, `list_overdue_tickets`, `list_unassigned_tickets`, `list_escalated_tickets`, `get_ticket_stats`, `get_customer_or_org_history`, `get_procurement_status`, and the one write tool `add_ticket_note`), `queries.ts` (implementations — every read re-runs `can()` + `ticketsVisibilityCondition(user)`; `add_ticket_note` can only post an internal note, never a customer-facing reply) — see §23 |
...
| `sla-monitor` | cron `*/5 * * * *` | Scans in-flight tickets, marks 50% / 80% / 100% transitions exactly once via the dedicated stamp columns. Skips tickets with `sla_paused_at` set (§13). 50% is in-app only; 80%+ adds SMS via the dispatcher; breaches audit `ticket.sla_breach` |
...
| `categories.ts` / `ticket-types.ts` | CRUD for `ticket_categories` / `ticket_types` (create, rename, reorder, set-default, deactivate — never hard-delete so historical tickets keep their label). Gated on `settings.update` |
...
| `mcp-tokens.ts` | `generateMcpToken` (shown once, prefix `axmcp_`), `revokeMcpToken`, `listMcpTokens` — self-service, user-owned (§23) |
...
- `ui/` — `button`, `input`, `card`, `dropdown-menu`, `badge`, `skeleton`, `avatar`, `separator`, `label`, `tooltip`, `textarea`, `dialog`, `table`, `row-actions`, `rich-text-editor` (TipTap), `pagination`, `page-size-select`, `select`, `spinner`, `url-filter-select`, `url-search-input`, `data-table` + `data-table-column-header` (the generic sortable/filterable table — column `meta.filter: {kind:"enum"|"dateRange", ...}` drives per-column filter UI; backs both `tickets-table` and `work-log-table`, having superseded the older `MultiSelect`/`FilterCombobox` approach)
- `shared/` — `topbar`, `sidebar` (each `NAV_ITEM` declares the `Permission` it requires; the gated layout passes the caller's permission array and the sidebar filters before rendering so a user never sees a link they can't reach), `profile-menu`, `notification-bell`, `global-search` (⌘K palette), `skip-link`, `impersonation-banner`, `reauth-modal` + `use-reauth-gate`, `export-menu` (the shared CSV/XLSX/PDF dropdown, §16, reused on tickets/organizations/roles/users/procurement/audit/reports)
...
- `settings/` — `branding-form`, `business-hours-form`, `sla-form`, `holidays-list`, `string-list-form`, `scalar-form`, `rate-limit-form`, `save-button`, `taxonomy-table` + `add-taxonomy-button` (shared by the categories and types admin pages)
...
- **IT Director** — read-mostly: ticket view/update/assign/reply/internal_note/deescalate/**close** + organizations.view + reports.view + audit.view
- **Coordinator** — operational: ticket triage incl. resolve_skip_note/**close**; organizations.view/create/update; procurement view/manage; users.view; reports.view
- **Technician** — owns their assigned tickets: ticket view/update/assign(own)/reply/internal_note/resolve/escalate; procurement view/create/update
...
- **Monitor** (`inngest/functions/sla-monitor.ts`): every 5 minutes, scans in-flight tickets, marks 50% / 80% / 100% transitions exactly once via `sla_warning_50_at`, `sla_warning_80_at`, `sla_breached_at`. The DB column NULL-check is what makes it idempotent. Tickets with `sla_paused_at` set are skipped entirely.
...
`src/app/api/mcp/route.ts` exposes a [Model Context Protocol](https://modelcontextprotocol.io) server (`@modelcontextprotocol/sdk`) so an AI agent (e.g. Claude) can be pointed at the ticketing system directly, authenticated as a specific user rather than through the browser session.

1. A user mints a Bearer token from their own `/admin/profile` (`generateMcpToken`, prefix `axmcp_…`, shown once) — self-service, revocable, no admin action required. Tokens are stored SHA-256-hashed in `mcp_tokens` (migration `0030`).
2. Every request to `/api/mcp` presents the token as `Authorization: Bearer axmcp_…`. `resolveMcpToken` (`lib/auth/mcp-tokens.ts`) hashes it, looks it up (`revoked_at IS NULL`), and rebuilds a full `SessionUser` from the DB — so **an MCP token can never do more than the user who minted it already could**; every tool call re-runs `can()` and `ticketsVisibilityCondition(user)`.
3. `lib/mcp/server.ts` registers the tool set: `get_ticket`, `list_tickets_by_status`, `list_overdue_tickets`, `list_unassigned_tickets`, `list_escalated_tickets`, `get_ticket_stats`, `get_customer_or_org_history`, `get_procurement_status` — all read-only.
4. The sole write tool, `add_ticket_note`, can only post an **internal note** — never a customer-facing reply, regardless of what permissions the underlying user holds.
