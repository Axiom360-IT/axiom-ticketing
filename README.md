...
- `(gated)/` — every route below this segment is protected by `layout.tsx`, which (a) requires a valid Better Auth session, (b) enforces the 12-hour idle timeout by inspecting `session.updatedAt`, (c) renders the sidebar + topbar + impersonation banner, and (d) hosts the `<main id="main-content">` landmark
  ...
  - `tickets/`, `tickets/new`, `tickets/[id]` — the list page's primary split is a row of **type sub-tabs** (an icon + label per active `ticket_types` row, plus "All types"), rebuilt from the admin-managed taxonomy on every request so a newly-added type gets a tab automatically; the table itself has no Type column (the tab you're on IS the type filter). Active/Closed/All view tabs + the Awaiting Customer/On Hold quick chips sit in their own row below that
...
- `settings/` — `branding-form`, `business-hours-form`, `sla-form`, `role-checklist-form` (generic staff-role checklist persisted as a `string[]` setting; backs the SLA warning/breach notify-role pickers, §13), `holidays-list`, `string-list-form`, `scalar-form`, `rate-limit-form`, `save-button`, `taxonomy-table` + `add-taxonomy-button` (shared by the categories and types admin pages)
...
| `categories-types-write.ts` | `create_ticket_category` / `rename_ticket_category` / `set_ticket_category_active` / `set_default_ticket_category` / `delete_ticket_category`, and the same 5 for ticket **types**. Reordering is UI-only, skipped here. |
